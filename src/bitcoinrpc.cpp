...
{
    return GetBoolArg("-testnet", false) ? 55887 : 55888;
}
...
void PrintConsole(const char* format, ...)
{
    ...
    MyMessageBox(buffer, "Loopcoin", wxOK | wxICON_EXCLAMATION);
#else
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        CScript scriptPubKey;
        if (!scriptPubKey.SetBitcoinAddress(strAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Loopcoin address:")+strAddress);
...
Value importaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importaddress <loopcoinaddress> [label] [rescan=true]\n"
...
#else
    if (fUseSSL)
        throw runtime_error("-rpcssl=1, but loopcoin compiled without full openssl libraries.");
#endif
...
int CommandLineRPC(int argc, char *argv[])
{
    ...
    if (strPrint != "")
    {
        ...
        MyMessageBox(strPrint, "Loopcoin", wxOK);
#else
