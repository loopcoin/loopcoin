...
string SendMoneyWithInputTx(const CScript& scriptPubKey, int64 nValue, int64 nNetFee, const CWalletTx& wtxIn, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (fAskFee && !uiInterface.ThreadSafeAskFee(nFeeRequired))
        return "ABORTED";
#else
    if (fAskFee && !ThreadSafeAskFee(nFeeRequired, "Loopcoin", NULL))
        return "ABORTED";
#endif
...
Value sendtoname(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoname <loopcoinname> <amount> [comment] [comment-to]\n"
    ...
    if (!AddressToHash160(strAddress, hash160))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No valid loopcoin address");
...
Value name_list(const Array& params, bool fHelp)
{
    ...
    CRITICAL_BLOCK(pwalletMain->cs_mapWallet)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, pwalletMain->mapWallet)
        {
            ...
            if (tx.nVersion != LOOPCOIN_TX_VERSION)
                continue;
...
Value name_firstupdate(const Array& params, bool fHelp)
{
    ...
    CWalletTx wtx;
    wtx.nVersion = LOOPCOIN_TX_VERSION;
  ...
  if (!IsValidBitcoinAddress (address))
    {
      ...
      msg << "Invalid Loopcoin address: " << address;
      throw JSONRPCError (RPC_INVALID_ADDRESS_OR_KEY, msg.str ());
    }

  tx.nVersion = LOOPCOIN_TX_VERSION;
...
CHooks* InitHook()
{
    ...
    hashGenesisBlock = hashLoopCoinGenesisBlock;
    printf("Setup loopcoin genesis block %s\n", hashGenesisBlock.GetHex().c_str());
    return new CLoopcoinHooks();
}

bool CLoopcoinHooks::IsStandard(const CScript& scriptPubKey)
{
    return true;
}
...
void CLoopcoinHooks::AddToWallet(CWalletTx& wtx)
{
}

bool CLoopcoinHooks::IsMine(const CTransaction& tx)
{
    if (tx.nVersion != LOOPCOIN_TX_VERSION)
        return false;
...
bool CLoopcoinHooks::IsMine(const CTransaction& tx, const CTxOut& txout, bool ignore_name_new /* = false*/)
{
    if (tx.nVersion != LOOPCOIN_TX_VERSION)
        return false;
...
void
CLoopcoinHooks::AcceptToMemoryPool (DatabaseSet& dbset, const CTransaction& tx)
{
    if (tx.nVersion != LOOPCOIN_TX_VERSION)
        return;
...
void CLoopcoinHooks::RemoveFromMemoryPool(const CTransaction& tx)
{
    if (tx.nVersion != LOOPCOIN_TX_VERSION)
        return;
...
bool GetNameOfTx(const CTransaction& tx, vector<unsigned char>& name)
{
    ...
    if (!good)
        return error("GetNameOfTx() : could not decode a loopcoin tx");
...
IsConflictedTx (DatabaseSet& dbset, const CTransaction& tx, vchType& name)
{
    ...
    if (!good)
        return error("IsConflictedTx() : could not decode a loopcoin tx");
    int nPrevHeight;
    int nDepth;
    int64 nNetFee;
...
bool
CLoopcoinHooks::ConnectInputs (DatabaseSet& dbset,
                               map<uint256, CTxIndex>& mapTestPool,
                               const CTransaction& tx,
                               vector<CTransaction>& vTxPrev,
                               vector<CTxIndex>& vTxindex,
                               ...
                               bool fBlock, bool fMiner)
{
    ...
    if (tx.nVersion != LOOPCOIN_TX_VERSION)
    {
        ...
        // would be lost
        if (found)
            return error("ConnectInputHook() : a non-loopcoin transaction with a loopcoin input");
        return true;
    }
    ...
    if (!good)
        return error("ConnectInputsHook() : could not decode a loopcoin tx");
    ...
    switch (op)
    {
        case OP_NAME_NEW:
            if (found)
                return error("ConnectInputsHook() : name_new tx pointing to previous loopcoin tx");
...
bool
CLoopcoinHooks::DisconnectInputs (DatabaseSet& dbset, const CTransaction& tx,
                                  CBlockIndex* pindexBlock)
{
    if (tx.nVersion != LOOPCOIN_TX_VERSION)
        return true;
    ...
    bool good = DecodeNameTx(tx, op, nOut, vvchArgs, pindexBlock->nHeight);
    if (!good)
        return error("DisconnectInputsHook() : could not decode loopcoin tx");
    if (op == OP_NAME_FIRSTUPDATE || op == OP_NAME_UPDATE)
    {
        //vector<CDiskTxPos> vtxPos;
        vector<CNameIndex> vtxPos;
...
bool CLoopcoinHooks::CheckTransaction(const CTransaction& tx)
{
    if (tx.nVersion != LOOPCOIN_TX_VERSION)
        return true;
...
bool CLoopcoinHooks::ExtractAddress(const CScript& script, string& address)
{
...
bool
CLoopcoinHooks::ConnectBlock (CBlock& block, DatabaseSet& dbset,
                              CBlockIndex* pindex)
{
    return true;
}
...
bool
CLoopcoinHooks::DisconnectBlock (CBlock& block, DatabaseSet& dbset,
                                 CBlockIndex* pindex)
{
    return true;
}
...
{
    if (!fTestNet)
        if ((nHeight == 1 && hash != uint256("0x0000000059fa4eccdd2628c61e2f19618ba701898a0d33dfb79ee5d590b7ee87")) ||
                (nHeight == 512 && hash != uint256("0x00000000ead9867310f287a47b08d61099a5dc4d985c9b84176b8318d71c9422")))
            return false;
    return true;
}

string CLoopcoinHooks::IrcPrefix()
{
    return "loopcoin";
}
