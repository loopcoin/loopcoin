...
bool AppInit2(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        string strUsage = string() +
          _("loopcoin version") + " " + FormatFullVersion() + "\n\n" +
          _("Usage:") + "\t\t\t\t\t\t\t\t\t\t\n" +
            "  loopcoin [options]                   \t  " + "\n" +
            "  loopcoin [options] <command> [params]\t  " + _("Send command to -server or loopcoind") + "\n" +
            "  loopcoin [options] help              \t\t  " + _("List commands") + "\n" +
            "  loopcoin [options] help <command>    \t\t  " + _("Get help for a command") + "\n";

        strUsage += "\n" + HelpMessage();

#if defined(__WXMSW__) && defined(GUI)
        // Tabs make the columns line up in the message box
        wxMessageBox(strUsage, "Loopcoin", wxOK);
#else
        // Remove tabs
...
//#if defined(__WXMSW__) && defined(GUI)
    ...
    if (psingleinstancechecker->IsAnotherRunning())
    {
        ...
        loop
        {
            ...
            HWND hwndPrev = FindWindowA("wxWindowClassNR", "Loopcoin");
            if (hwndPrev)
            {
    ...
    if (!lock.try_lock())
    {
        wxMessageBox(strprintf(_("Cannot obtain a lock on data directory %s.  loopcoin is probably already running."), GetDataDir().c_str()), "Loopcoin");
        return false;
    }
    ...
    if (!fNoListen)
    {
        ...
        {
            wxMessageBox(strErrors, "Loopcoin");
            return false;
        }
    }
    ...
    if (mapArgs.count("-mininput"))
    {
        ...
        {
            wxMessageBox(_("Invalid amount for -mininput=<amount>"), "Loopcoin");
            return false;
        }
    }
    ...
    if (!strErrors.empty())
    {
        wxMessageBox(strErrors, "Loopcoin", wxOK | wxICON_ERROR);
        return false;
    }
    ...
    if (mapArgs.count("-proxy"))
    {
        ...
        {
            wxMessageBox(_("Invalid -proxy address"), "Loopcoin");
            return false;
        }
    }
    ...
    if (mapArgs.count("-paytxfee"))
    {
        ...
        {
            wxMessageBox(_("Invalid amount for -paytxfee=<amount>"), "Loopcoin");
            return false;
        }
        if (nTransactionFee > 0.25 * COIN)
            wxMessageBox(_("Warning: -paytxfee is set very high.  This is the transaction fee you will pay if you send a transaction."), "Loopcoin", wxOK | wxICON_EXCLAMATION);
    }
    ...
    if (!CreateThread(StartNode, NULL))
        wxMessageBox("Error: CreateThread(StartNode) failed", "Loopcoin");
...
std::string HelpMessage()
{
    std::string strUsage = std::string(_("Options:\n")) +
        ...
        "  -rpcport=<port>  \t\t  " + _("Listen for JSON-RPC connections on <port> (default: 8337)\n") +
...
#ifdef USE_SSL
    strUsage += std::string() +
        _("\nSSL options: (see the loopcoin Wiki for SSL setup instructions)\n") +
