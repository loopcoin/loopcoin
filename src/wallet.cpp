...
#include "loopcoin.h"    // For DecodeNameScript in GetAmounts to correctly compute credit/debit
    ...
    CRITICAL_BLOCK(cs_mapWallet)
    {
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s lpc %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat) CRITICAL_BLOCK(cs_mapWallet)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (dbset.tx ().ReadTxIndex (wtx.GetHash (), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s lpc %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
string CWallet::SendMoneyToBitcoinAddress(string strAddress, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (!scriptPubKey.SetBitcoinAddress(strAddress))
        return _("Invalid loopcoin address");
