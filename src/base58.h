...
// everywhere (and called just CSecret); in the current Loopcoin implementation it is only used for
...
// Note: Loopcoin implementation uses a hack: CSecret32 is used here, everywhere else
...
public:
    void SetSecret(const CSecret32& vchSecret, bool fCompressed)
    {
        ...
        SetData(fTestNet ? 239 : 129, &vchSecret[0], vchSecret.size());
        if (fCompressed)
            vchData.push_back(1);
    }
