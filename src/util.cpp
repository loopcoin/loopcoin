...
#ifdef GUI
    if (wxTheApp && !fDaemon)
        MyMessageBox(pszMessage, "Loopcoin", wxOK | wxICON_ERROR);
#endif
    throw;
}
...
#ifdef GUI
    uiInterface.ThreadSafeMessageBox(strMessage, "Loopcoin", wxOK | wxICON_EXCLAMATION);
#else
    ThreadSafeMessageBox(strMessage, "Loopcoin", wxOK | wxICON_EXCLAMATION);
#endif
...
string GetDefaultDataDir()
{
    ...
    // Windows: C:\Documents and Settings\username\Application Data\Loopcoin
    // Mac: ~/Library/Application Support/Loopcoin
    // Unix: ~/.loopcoin
#ifdef __WXMSW__
    // Windows
...
{
    string confFile = GetConfigFile("loopcoin.conf");
...
{
    namespace fs = boost::filesystem;
    fs::path pathConfig(GetArg("-pid", "loopcoind.pid"));
    if (!pathConfig.is_complete())
        pathConfig = fs::path(GetDataDir()) / pathConfig;
    return pathConfig.string();
}
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    boost::thread(boost::bind(MyMessageBox, strMessage+" ", string("Loopcoin"), wxOK | wxICON_EXCLAMATION, (wxWindow*)NULL, -1, -1));
                }
            }
        }
